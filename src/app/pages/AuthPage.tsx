...
export function AuthPage({ mode }: Readonly<AuthPageProps>) {
  ...
  return (
    <main className="glass-panel auth-panel">
      <h1 className="auth-title">{t('app.title')}</h1>

      {!configured && (
        <div className="configuration-message" role="alert">
          <Trans
            t={t}
      ...
      <div className="tab-container" role="group" aria-label={t('auth.modeLabel')}>
        <NavLink to={routePaths.login} className={({ isActive }) => `tab ${isActive ? 'active' : ''}`} end>
          {t('auth.loginTab')}
        </NavLink>
        <NavLink to={routePaths.register} className={({ isActive }) => `tab ${isActive ? 'active' : ''}`} end>
          {t('auth.registerTab')}
        </NavLink>
      </div>
      ...
      <form onSubmit={(event) => form.submit(event, mode)} aria-label={t(isLogin ? 'auth.loginForm' : 'auth.registerForm')} aria-busy={form.loading}>
        ...
        <button className="btn-primary" type="submit" disabled={form.loading || !configured}>
          {submitLabel}
        </button>
      </form>

      {form.message && <div className="feedback-message" role="status">{form.message}</div>}
    </main>
  )
}
